...
public:
    ...
    virtual void display_info() const {
        ...
        std::cout << "Status: " << (activated ? _xor_decrypt((const unsigned char[]){0xde,0xfc,0xeb,0xf6,0xe9,0xfa}, 6, 0x9f) : _xor_decrypt((const unsigned char[]){0xa4,0x83,0x8c,0x8e,0x99,0x84,0x9b,0x88}, 8, 0xed)) << "\n";
    }
...
int main(int argc, char** argv) {
    ...
    if (valid && enterprise->is_activated()) {
        ...
        std::cout << "[CRYPTO] Signature valid: " << (sig_valid ? _xor_decrypt((const unsigned char[]){0xd0,0xec,0xfa}, 3, 0x89) : "No") << "\n";
