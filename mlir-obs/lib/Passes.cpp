...
void StringEncryptPass::runOnOperation() {
  ...
  // The format is: @llvm.global_ctors = appending global [1 x { i32, ptr, ptr }] [{ i32 65535, ptr @__obfs_init, ptr null }]
  if (!module.lookupSymbol<LLVM::GlobalOp>("llvm.global_ctors")) {
    ...
    // Create the global_ctors with an initializer region
    ...
    // Priority 65535 (low priority, runs after standard constructors)
    ...
    // Create struct: { priority, func_ptr, null }
