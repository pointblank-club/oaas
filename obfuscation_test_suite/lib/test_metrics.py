...
def analyze_complexity(baseline: str, obfuscated: str) -> Dict[str, Any]:
    ...
    return {
        ...
        'changes': {
            ...
            'symbol_reduction_percent': _compute_percent_change(baseline_syms, obf_syms, inverse=True)
        }
    }
...
def _analyze_objdump(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for line in result.split('\n'):
            ...
            if any(x in line for x in ['jmp', 'je', 'jne', 'jz', 'jnz', 'jg', 'jl']):
        ...
        # Estimate basic blocks (rough heuristic)
        metrics['basic_blocks_estimate'] = max(1, metrics['branch_instructions'] // 2)
...
def _compute_percent_change(old_val: float, new_val: float, inverse: bool = False) -> float:
    ...
    if inverse:
        # For metrics where decrease is good (like symbols)
        change = 100 * (old_val - new_val) / old_val
    else:
        # For metrics where increase might be expected (like size)
